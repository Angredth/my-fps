const gun = new THREE.Group();
    ...
    function bevelBox(name, size, pos, mat, rot = [0, 0, 0], radius = 0.08, parent = gun) {
      // 기본 BoxGeometry를 중심으로, 가장자리 보조 박스를 덧대어 둥근 느낌만 냅니다.
      const g = new THREE.Group();
      g.name = name;
      parent.add(g);
      box(`${name}_core`, size, [0, 0, 0], mat, [0, 0, 0], g);
      const edgeMat = mat;
      box(`${name}_top_soft`, [size[0] * 0.95, radius, size[2] * 0.92], [0, size[1] / 2 + radius * 0.18, 0], edgeMat, [0, 0, 0], g);
      box(`${name}_bottom_soft`, [size[0] * 0.95, radius, size[2] * 0.92], [0, -size[1] / 2 - radius * 0.18, 0], edgeMat, [0, 0, 0], g);
      g.position.set(pos[0], pos[1], pos[2]);
      g.rotation.set(rot[0], rot[1], rot[2]);
      return g;
    }

    // 좌우 방향: X축, 위아래: Y축, 두께: Z축
    // 전체적으로 FN 5-7의 실루엣만 따온 비기능성 게임용 프롭 모델입니다.
    ...
    // 슬라이드 상단/측면 형상 보조
    box("ejection_port_cut_dark", [1.15, 0.16, 0.77], [0.35, 1.37, 0], mats.black, [0, 0, -0.02]);
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 4; col++) {
        const x = -1.88 + col * 0.17 + row * 0.015;
        const y = -0.55 - row * 0.18;
        box(`grip_check_${row}_${col}_L`, [0.075, 0.075, 0.035], [x, y, 0.42], mats.frameDark, [0, 0, 0.78]);
      }
    }
    for (let i = 0; i < 9; i++) {
    ...
    const screwPositions = [
      [-2.9, 0.46, 0.43], [-2.35, 0.4, 0.43], [-0.02, 0.08, 0.43],
      [0.72, 0.27, 0.43], [1.72, 0.28, 0.43], [2.7, 0.12, 0.43]
    ];
    ...
    // 측면 각인 느낌: 실제 텍스트 대신 얇은 박스 줄로 처리
